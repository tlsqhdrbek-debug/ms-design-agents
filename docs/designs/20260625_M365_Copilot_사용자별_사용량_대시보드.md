...
## 9. 시각화·전달 — WPF exe + Dataverse Web API 직접 호출

### 9.1 환경 확정 (사용자 정정 반영)
- 회사는 **표준 M365 테넌트 사용 중** (양망 분리 환경 아님)
- 회사 PC에서 Microsoft Cloud (Dataverse, Graph 등) **정상 접근 가능**
- exe는 외부 개발 환경(또는 사내 빌드 머신)에서 빌드 → 회사 PC 배포 → 회사 PC가 보유한 사용자 자격증명으로 OAuth → Dataverse 직접 호출
- **Power BI 사용 불가** (정책) — 그래서 자체 exe

### 9.2 데이터 연결 — Dataverse Web API 직접 호출 (채택)

```mermaid
flowchart LR
    USER[사용자 PC<br/>cockpit.exe WPF]
    ENTRA[Entra ID<br/>OAuth2 token]
    DV[(Dataverse<br/>회사 환경<br/>m365copilotcockpit.crm4.dynamics.com)]

    USER -- 1: Interactive Sign-in (MSAL) --> ENTRA
    ENTRA -- 2: Access Token (scope: {orgUrl}/.default) --> USER
    USER -- 3: GET /api/data/v9.2/msd_chat_usage_dailies?$filter=... --> DV
    DV -- 4: JSON rows --> USER
```

### 9.3 권한 모델 — 2단계 (먼저 이해)

> exe가 Dataverse 데이터를 보려면 **두 종류의 권한**이 모두 필요. 사용자가 흔히 헷갈리는 부분이라 비유로 먼저 정리.

| 단계 | 무엇 | 비유 | 어디서 설정 |
| --- | --- | --- | --- |
| **A. Entra App permission** | "이 exe가 Dataverse를 호출할 수 있나" — 앱 단위 허가 | 사옥 출입증 | Entra admin center > App registrations |
| **B. Dataverse 보안 역할** | "이 사용자가 Dataverse 안에서 어떤 테이블을 읽을 수 있나" — 사용자 단위 허가 | 사무실 키 | Power Platform admin center > 환경 > Users + permissions |

→ 둘 다 있어야 함. 출입증만 있고 사무실 키 없으면 사옥엔 들어와도 정작 데이터엔 못 닿는다.

### 9.4 셋업 절차 — 4단계

#### Step 1. Entra App 등록 (App permission 준비)
> Entra admin center > Identity > Applications > **App registrations** > New registration

| 입력 항목 | 값 |
| --- | --- |
| Name | `M365 Cockpit Viewer` |
| Supported account types | **Accounts in this organizational directory only (Single tenant)** |
| Redirect URI 플랫폼 | **Public client/native (mobile & desktop)** |
| Redirect URI 값 | `http://localhost` |

등록 후:
- Overview 페이지에서 **Application (client) ID** 와 **Directory (tenant) ID** 두 값을 메모 (exe 코드에 박을 값)
- Authentication 탭 > "Allow public client flows" = **Yes**

> Phase 1의 `M365 Cockpit Collector` (Application — 백엔드 수집용)와 **완전 별개**. 이건 사용자가 직접 인증하는 Public Client.

#### Step 2. API Permission 부여 (Step 1의 App에 추가)
> 같은 App > API permissions > Add a permission

| 항목 | 값 |
| --- | --- |
| API 선택 | **Dynamics CRM** (목록에서 검색) |
| 권한 유형 | **Delegated permissions** |
| 권한 이름 | **`user_impersonation`** (Access Common Data Service as organization users) |

추가 후:
- **Grant admin consent for {tenant}** 버튼 클릭 (Global Admin 또는 Application Administrator만 가능)
- 상태가 ✅ Granted 로 바뀌면 완료

#### Step 3. Dataverse 보안 역할 만들기 + 사용자에게 부여

> Power Platform admin center (https://admin.powerplatform.microsoft.com) > 본 Cockpit 환경 선택

**3-1. 보안 역할 생성** (Settings > Users + permissions > Security roles > New role)

| 항목 | 값 |
| --- | --- |
| Name | `M365 Cockpit Viewer` |
| 권한 | 다음 6개 테이블 모두 **Read**(User 또는 Organization 범위) |

테이블:
- `msd_user`, `msd_agent`, `msd_copilot_usage_daily`, `msd_chat_usage_daily`, `msd_agent_usage_daily`, `msd_credit_daily`

다른 권한 (Create/Update/Delete/Append/AppendTo)은 모두 **빈 칸**. Read-only.

**3-2. 사용자에게 역할 부여** (Users + permissions > Users)
- 보고서를 볼 사용자 선택 → **Manage security roles** → `Basic User` + `M365 Cockpit Viewer` 체크 → Save
- 사용자가 추가될 때마다 같은 절차 반복 (Phase 1 운영자 일과에 포함)

> **(옵션, 나중에)** 사용자 수가 늘면 AD 그룹 → Dataverse "Group team" 매핑으로 자동화. 초기엔 수동 부여로 충분.

#### Step 4. exe 코드 (Client ID + Tenant ID + Org URL)

```csharp
const string ClientId = "Step 1에서 받은 Client ID";
const string TenantId = "Step 1에서 받은 Tenant ID";
const string OrgUrl   = "https://yourorg.crm4.dynamics.com";  // 본인 환경 URL
...
var scopes = new[] { $"{OrgUrl}/.default" };
var result = await pca.AcquireTokenInteractive(scopes).ExecuteAsync();
// → 첫 실행 시 브라우저 팝업으로 사용자 로그인 (SSO 환경이면 자동 통과)
// → 이후엔 AcquireTokenSilent로 무인지 갱신
```

(전체 호출 코드는 9.5 참조)

### 9.5 WPF 구현 핵심 코드 (MSAL.NET + Dataverse REST)

NuGet 패키지:
- `Microsoft.Identity.Client` (MSAL.NET)
- (옵션) `Microsoft.Identity.Client.Extensions.Msal` — 토큰 캐시 영속화

```csharp
// 1) MSAL 초기화
var pca = PublicClientApplicationBuilder
    .Create(ClientId)                                  // Entra App "M365 Cockpit Viewer" 의 Client ID
    .WithAuthority(AzureCloudInstance.AzurePublic, TenantId)
    .WithRedirectUri("http://localhost")
    .Build();

// 2) 토큰 캐시 (사용자 PC에 DPAPI로 암호화 저장)
var storageProps = new StorageCreationPropertiesBuilder(
        "msal_cache.dat",
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "M365Cockpit"))
    .Build();
var cacheHelper = await MsalCacheHelper.CreateAsync(storageProps);
cacheHelper.RegisterCache(pca.UserTokenCache);

// 3) 토큰 획득 (SSO 시도 → 실패 시 interactive)
var scopes = new[] { $"{OrgUrl}/.default" };  // OrgUrl = https://{org}.crm4.dynamics.com
...
catch (MsalUiRequiredException)
{
    result = await pca.AcquireTokenInteractive(scopes)
                      .ExecuteAsync();   // 사용자 브라우저 팝업 1회
}

// 4) Dataverse Web API 호출
using var http = new HttpClient { BaseAddress = new Uri($"{OrgUrl}/api/data/v9.2/") };
...
var resp = await http.GetAsync(
    "msd_chat_usage_dailies?" +
    "$select=msd_upn,msd_usagedate,msd_promptssubmitted,msd_departmentsnapshot&" +
    $"$filter=msd_usagedate ge {DateTime.UtcNow.AddDays(-30):yyyy-MM-dd}");
var body = await resp.Content.ReadAsStringAsync();
// JSON → ChatUsage[] 역직렬화 → 차트로 렌더
```

### 9.6 WPF vs Electron 비교

| 항목 | WPF (.NET) | Electron (Node + Chromium) |
| --- | --- | --- |
| MSAL 라이브러리 | MSAL.NET (성숙) | MSAL Node (사용 가능) |
| 토큰 캐시 | DPAPI 자동 암호화 (Windows 표준) | manual 처리 필요 |
| 코드서명 | EV 인증서 표준 | 동상 |
| 차트 라이브러리 | LiveCharts2 또는 OxyPlot | Chart.js 그대로 |
| 배포 사이즈 | ~50 MB | ~150 MB (Chromium 번들) |
| 사내 .NET 환경 호환 | ✅ 사실상 표준 | 별도 런타임 |
| **추천** | **WPF 강력 권장** (MSAL.NET + DPAPI 통합 우수) | 웹 자산 재사용 큰 경우만 |

### 9.7 화면 구성 (단일 exe, SPA 패턴)

```
┌──────────────────────────────────────────────────────────────────────────────┐
│  M365 Copilot Cockpit                        alice@contoso.com  [새로고침]    │
│                                              마지막 조회: 2026-06-25 14:30    │
├──────────────────────────────────────────────────────────────────────────────┤
│  ┌─KPI──────┐ ┌─KPI──────┐ ┌─KPI──────┐ ┌─KPI──────┐ ┌─KPI──────┐           │
│  │ Active   │ │ Chat     │ │ Agent    │ │ Credit   │ │ Top Agent│           │
│  │ 1,240    │ │ 18,234p  │ │ 4,561r   │ │ ₩2.3M    │ │ HR FAQ   │           │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘           │
├──────────────────────────────────────────────────────────────────────────────┤
│ [Overview] [Users] [Departments] [Agents] [Credits]                          │
├──────────────────────────────────────────────────────────────────────────────┤
│  Users 탭:                                                                    │
│  - Top 20 헤비 유저 표 + 검색                                                  │
│  - 사용자 클릭 → 상세 (Surface별 활동, Agent별 사용량, Credit 추이 라인 차트) │
└──────────────────────────────────────────────────────────────────────────────┘
```

### 9.8 성능·캐싱

- **첫 로딩**: 4개 Fact 테이블 30일치 병렬 호출 → 메모리 보관 (~수만 행, MB 단위)
- **새로고침**: 마지막 조회 시간 이후 변경분만 (`modifiedon ge ?`)
- **사용자 페이지 클릭**: 메모리에서 즉시 표시 (DB 재호출 없음)
- **토큰 캐시**: `%LOCALAPPDATA%\M365Cockpit\msal_cache.dat` (MSAL Extensions DPAPI 암호화)
- **결과 캐시** (옵션): SQLite 또는 LiteDB로 마지막 조회 결과 보관 → 오프라인 30일 view-only

### 9.9 배포·업데이트

| 항목 | 결정 |
| --- | --- |
| 패키징 | MSI (`dotnet publish` + WiX 또는 MSIX) |
| 코드서명 | 회사 EV 인증서 |
| 배포 채널 | Intune Win32 앱 (또는 SCCM) |
| 자동 업데이트 | ClickOnce 또는 Squirrel.Windows |
| 업데이트 주기 | 분기 1회 (UI/스키마 변경 시) |

> 데이터는 매번 실시간 조회이므로 exe 재배포와 무관.

### 9.10 보안 체크리스트

- exe 코드서명 (EV) + 사내 AV 화이트리스트 등록
- OAuth 토큰 DPAPI 암호화 (MSAL Extensions 자동)
- RBAC = **Dataverse 보안 역할** (사용자별 read-only)
- Conditional Access (회사 PC + MFA 강제 권장)
- TLS 1.2+ (Dataverse 기본)
- exe 로컬 로그 (`%APPDATA%\M365Cockpit\logs\`) — 사용자명·조회 시각·오류만, 데이터 본문 X
- 변경 이력: GitHub 소스 관리, 분기 1회 보안팀 회독

### 9.11 보조 패턴 — 오프라인 캐시 (옵션)

- 출장·외근 시 Dataverse 접근 불가 가능성 대비
- exe가 매번 조회 시 결과를 로컬 SQLite/LiteDB에 적재
- 오프라인 모드: "마지막 조회 결과 (캐시)" 표시
- 캐시 만료 7일 (이후 강제 새로고침)
- DPAPI 또는 BitLocker로 캐시 암호화

### 9.12 미채택 옵션 정리 (변경 이력)

| 옵션 | 미채택 이유 |
| --- | --- |
| 9-A. JSON 스냅샷 (이전 권장) | 양망 분리 가정 — 사용자 환경엔 해당 없음 |
| 9-C. Azure Function 게이트웨이 | 추가 인프라 부담. exe 직접 호출이 더 단순 |

> v2.2 r1에서 9-A를 권장했으나, 사용자 환경 정정(표준 M365, 양망 분리 없음)으로 **9-B Dataverse 직접 호출이 메인**으로 확정.

---

## 10. 참고 (Microsoft Learn)
