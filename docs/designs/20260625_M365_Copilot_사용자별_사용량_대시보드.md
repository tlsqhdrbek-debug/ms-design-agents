...
## 9. 시각화·전달 — WPF/Electron exe 패턴 (신규)

### 9.1 배경 (사용자 결정 반영)
- 데이터 수집·축적: 외부망 Dataverse (Phase 1 플로우)
- 표시 화면: **WPF 또는 Electron exe**. 외부망에서 빌드 → 양망 게이트(Type C 파일 반입)로 반입 → 내부망 사용자 PC 배포
- **Power BI 사용 불가** (정책)
- 내부망 표시 (외부 네트워크 호출 0)

### 9.2 데이터 연결 옵션 3가지

| 옵션 | exe 측 동작 | 데이터 신선도 | 양망 호환 | 인증 |
| --- | --- | --- | --- | --- |
| **9-A. JSON 스냅샷 fetch** (✅ 권장) | exe가 사내 파일서버의 daily JSON을 fetch + 캐시 | D+1 | ✅ 완벽 | 파일서버 ACL |
| 9-B. Dataverse Web API 직접 호출 | MSAL/OAuth로 외부 Dataverse 직접 | 실시간 | ❌ 양망 정책 위반 가능성 | OAuth Device Code |
| 9-C. Azure Function 게이트웨이 | API 중계 + 캐싱 | 거의 실시간 | ❌ 동상 | API Key/OAuth |

→ **9-A 채택**.

### 9.3 9-A 흐름 (Type C 파일반입 패턴 유지)

```mermaid
flowchart LR
    subgraph EXT["외부망 (M365 Tenant + Power Platform)"]
        DV[(Dataverse Fact 4)]
        F5[F-5 Export Flow<br/>매일 03:30 KST]
        SP[/SharePoint Online<br/>/Cockpit/Export/.../]
        DV --> F5 --> SP
    end

    subgraph GATE["양망 파일반입 게이트 (Type C)"]
        G[AV + DLP + SHA256 검증]
    end

    subgraph INT["내부망"]
        FS[/파일서버<br/>//fs/m365cockpit/data//]
        EXE[cockpit.exe<br/>WPF or Electron]
    end

    SP --> G --> FS
    EXE -. fetch snapshot_YYYYMMDD.json.gz .-> FS
```

### 9.4 F-5 Export 플로우 (신규)

**트리거**: Recurrence 매일 03:30 KST (Phase 1의 F-0~F-4 적재 완료 후).

| # | 액션 | 상세 |
| --- | --- | --- |
| 1 | List rows — msd_copilot_usage_daily | `$filter=msd_usagedate ge today-30` |
| 2 | List rows — msd_chat_usage_daily | 동상 |
| 3 | List rows — msd_agent_usage_daily | 동상 + msd_agent join |
| 4 | List rows — msd_credit_daily | 동상 |
| 5 | List rows — msd_user | 부서·라이선스 필터용 |
| 6 | Compose — Gold 집계 | 사용자별/부서별/Agent별/일별 KPI (PA 식) |
| 7 | Compose — JSON 직렬화 | `{meta, kpis, perUser, perDept, perAgent, perDay, sourceFootnotes}` |
| 8 | (옵션) Azure Function 호출 — gzip 압축 | PA 자체 gzip 미지원 시 단일 함수 1줄 |
| 9 | Compose — SHA-256 hash | 무결성 검증용 |
| 10 | Create file — SharePoint | `/Cockpit/Export/snapshot_YYYYMMDD.json.gz` + `.sha256` |
| 11 | (Failure) Teams 알림 | "Export 실패: @{outputs('...').statusCode}" |
...
```json
{
  "meta": {
    ...
    "tenantId": "...",
    "generatedUtc": "2026-06-25T18:30:00Z",
    "sha256": "...",
    "rowCounts": { "copilotUsage": 2410, "chat": 1832, "agent": 4561, "credit": 213 }
  },
  "kpis": {
    "activeUsers30d": 1240,
    "topAgent": { "name": "HR FAQ Bot", "users": 320 },
    "creditsMonth": 1234567,
    "newHireAdoption": 0.62
  },
  "perUser":  [ { "upn": "alice@contoso.com", "dept": "영업1팀", "copilot": {...}, "chat": {...}, "agents": [...], "credits": 0 } ],
  "perDept":  [ { "dept": "영업1팀", "active": 220, "promptsTotal": 18234, "creditsTotal": 0 } ],
  "perAgent": [ { "agentGuid": "1a2b...", "name": "HR FAQ Bot", "users": 320, "responses": 4231, "creditsMonth": 38000 } ],
  "perDay":   [ { "date": "2026-06-24", "dauChat": 412, "dauAgent": 188 } ],
  "sourceFootnotes": {
    "copilotUsage": "Graph getMicrosoft365CopilotUsageUserDetail",
    "agentResponses": "Admin Center Agents Report (preview) or Audit RecordType=261",
    "credits": "Admin Center Credits Report (preview) — 미라이선스 Chat metered만"
  }
}
```
...
| --- | --- | --- |
| 코드서명 | EV 인증서 표준 | 동상 |
| AV 화이트리스트 통과 | 비교적 용이 | Chromium 번들로 사이즈↑, 점검 부담 |
| 차트 라이브러리 | LiveCharts2 또는 OxyPlot | Chart.js 그대로 (외부망 빌드본 번들) |
| 배포 사이즈 | ~50 MB | ~150 MB |
| 사내 .NET 환경 호환 | ✅ 사실상 표준 | 별도 런타임 |
| 개발 인력 친숙도 (사내) | 일반적으로 ↑ | 웹 인력일 때만 ↑ |
| **추천** | **사내 표준 .NET이면 WPF** | 웹 자산 재사용·UX 풍부함 우선 시 |

### 9.7 코드서명·배포·업데이트

- **코드서명**: 회사 EV 인증서 → 외부망에서 sign → 양망 게이트 무결성 검증 통과 용이
- **배포 채널**:
  - **Intune** (Win11): MSI/MSIX 푸시
  - **SCCM** (레거시): MSI
  - **USB/공유폴더 수동**: 백업 채널
- **업데이트 분리**:
  - exe 자체 = 분기 1회 (UI 변경 시)
  - **JSON 데이터 = 매일 별도 반입** (exe 재배포 불필요)
  - 같은 cockpit.exe로 어제·오늘·내일 모든 JSON 표시 가능 → 동결된 자료체 보존 쉬움

### 9.8 exe 측 구현 가이드라인

- **외부 네트워크 호출 0** (정책 안전)
- 파일서버 경로: 환경변수(`%M365COCKPIT_DATA%`) 또는 첫 실행 시 설정 저장
- JSON gzip 해제:
  - WPF: `System.IO.Compression.GZipStream`
  - Electron: 내장 `zlib`
- 차트 라이브러리:
  - WPF: LiveCharts2 (NuGet, 라이선스 호환)
  - Electron: Chart.js v4 (단일 번들)
- 사용자 인증: 별도 로그인 X — Windows AD 로그인 사용자 그대로 표시
- 로깅: 로컬 로그 (Event Viewer 또는 `%APPDATA%\M365Cockpit\logs\`)
- 데이터 신선도 표시: 상단 우측 `Snapshot: 2026-06-25 (D-1)` 고정
- 데이터 누락 시: 해당 카드 회색 + "데이터 미수신" 안내

### 9.9 화면 구성 (단일 exe, SPA 패턴)

```
┌──────────────────────────────────────────────────────────────────────────────┐
│  M365 Copilot Cockpit                          Snapshot: 2026-06-25 (D-1)    │
├──────────────────────────────────────────────────────────────────────────────┤
│  ┌─KPI──────┐ ┌─KPI──────┐ ┌─KPI──────┐ ┌─KPI──────┐ ┌─KPI──────┐           │
...
├──────────────────────────────────────────────────────────────────────────────┤
│  Users 탭:                                                                    │
│  - Top 20 헤비 유저 표 (UPN, 부서, 30일 활동, Top Agent)                       │
│  - 사용자 검색 → 상세 페이지 (Surface별 활동, Agent별 사용량, Credit 추이)    │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
- exe 코드서명 + 해시 검증
- JSON 파일 SHA-256 무결성 검증 (meta.sha256와 대조)
- exe는 **읽기 전용** — 사용자 데이터 수정/송신 기능 0
- Sensitivity Label "Confidential / Internal" — JSON, exe 모두
- AV 통과 보장: 외부망에서 빌드 → 양망 게이트 AV + DLP 통과
- 변경 이력: GitHub로 exe 소스 관리, 분기 1회 보안팀 회독

### 9.11 옵션 9-B (Dataverse 직접 호출) — 미채택 이유 명시
- 내부망 PC → 외부망 Dataverse OAuth 호출은 본 환경 양망 정책상 제한
- 토큰 캐싱·갱신 처리 복잡 (특히 Device Code Flow)
- 망 차단 시 화면 즉시 깨짐 (오프라인 동작 X)
- 향후 양망 정책 완화 시에만 재검토

### 9.12 옵션 9-C (Azure Function 게이트웨이) — 미채택 이유
- 캐싱·필터링·집계 추가 가능하나, 내부망 → 외부 Function 호출은 동일 양망 제약
- 본 v2.2 메인 권고 X. 차후 v3 옵션 후보.
