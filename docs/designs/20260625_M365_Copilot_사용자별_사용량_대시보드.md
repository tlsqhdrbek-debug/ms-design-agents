...
### 9.3 Entra App 등록 (exe 전용 신규)

**App 이름**: `M365 Cockpit Viewer (Public Client)`
- Account types: **Single tenant**
- Platform: **Mobile and desktop applications**
- Redirect URI: `http://localhost` (loopback, native client 권장)
- **Allow public client flows = Yes** (Interactive + Device Code 허용)
- API permissions (Delegated):
  - **Dynamics CRM `user_impersonation`** ← 핵심. Dataverse에 사용자 자격으로 호출
- Admin consent 1회 (테넌트 관리자)
- 권장: Conditional Access — 회사 PC + MFA 강제

> Phase 1의 `M365 Cockpit Collector` (Application — 백엔드용)와 **별개**. 이건 사용자가 직접 인증하는 Public Client.

### 9.4 사용자 권한 (Dataverse 보안 역할)

각 사용자에게 회사 Dataverse 환경의 보안 역할 부여:
1. Basic User (기본)
2. **M365 Cockpit Viewer** (사용자 정의 역할, Read-only):
   - `msd_user` Read
   - `msd_agent` Read
   - `msd_copilot_usage_daily` Read
   - `msd_chat_usage_daily` Read
   - `msd_agent_usage_daily` Read
   - `msd_credit_daily` Read

권장: AD 그룹 `M365-Cockpit-Viewers` → 보안 역할 자동 할당 (Power Platform admin center의 그룹 팀 매핑)
