...
> Teams 녹화가 끝나면 참석자가 대본(transcript)을 복사해 에이전트에 붙여넣고(또는 파일로 올리고), 회사 양식 기반 회의록이 자동 작성되어 Word(.docx)로 저장된다. 저장 후 "이 문서를 누구에게 보내시겠습니까?"라고 묻고, 사용자가 **전사 직원 중 수신자를 검색·선택**하면 선택한 사용자만 문서를 열람할 수 있도록 권한을 부여하고, Teams로 공유 알림이 간다. 모두가 보는 SharePoint에 회의록이 노출되는 것을 원치 않는다. 누구나 쉽게 사용할 수 있어야 한다. 외부망 M365 테넌트 기준.
...
| 수신자 목록 | 이메일/UPN | **전사 디렉터리 검색 후 선택** | ✅ |
...
| 컴포넌트 | 역할 | 위치 | 사용 기술 |
|---------|------|------|-----------|
| 회의록 도우미 에이전트 | 대화·입력·초안·검토·수신자 검색 | 외부망 | Copilot Studio (Teams 채널) |
...
| 8 | **수신자 검색 루프** | 전사 직원 검색·선택(§5.5) | → `RecipientsCsv` |
...
### 5.5 수신자 — 전사 직원 검색 (Office 365 Users)
대본 참석자만이 아니라 **전 직원을 이름·메일로 검색**해 선택한다.

- 액션: **Office 365 Users — Search for users (V2)** (`searchTerm`으로 표시이름·성·이름·메일·메일닉네임·UPN 검색, 결과로 DisplayName·Mail·UPN·부서 등 반환). 토픽에서 커넥터 액션으로 직접 호출하거나 전용 조회 플로우로 래핑.
- 흐름(루프):
  1. Question: "받는 사람 이름이나 이메일 일부를 입력하세요." → `SearchTerm`
  2. Action(Search for users V2): `SearchTerm` → 일치 직원 목록
  3. Question(다중 선택/Adaptive Card): 후보(이름·부서) 표시 → 선택 → 선택자 UPN을 `RecipientsCsv`에 추가
  4. Question: "더 추가할까요? (추가 / 완료)" → 추가면 1로 복귀
- 게스트 계정(외부 사용자)·비활성 계정은 후보에서 제외. 대본 참석자는 첫 화면 **기본 후보**로 제시해 편의 제공.
...
- **F. 수신자 선택 UX** — 전사 검색(§5.5) + 참석자 기본 후보 + 게스트 제외.
...
5. **에이전트·토픽** — §5.2 노드. ④ Prompt, ⑧ 검색 루프, ⑨ Action 매핑.
...
| 2026-05-28 | 전사 직원 검색(Office 365 Users)·파일 업로드 입력·긴 대본 청크 최적화 추가, 권한 상속 차단 쉬운 설명 및 고유범위(5,000/50,000) 관리·연도별 라이브러리 보강 | ms-design-agents |
