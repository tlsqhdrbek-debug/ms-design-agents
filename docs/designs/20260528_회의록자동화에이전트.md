...
| 8 | **수신자 선택 카드 루프** | 검색창 카드 → 디렉터리 검색 → 결과 체크리스트 카드(§5.5) | → `Global.RecipientsCsv` |
...
| SearchResults | Table | 검색된 직원 목록 |
| selectedRecipients | Text | 이번 라운드 선택(콤마 구분) |
| **Global.RecipientsCsv** | Text | **누적 수신자 통(루프 유지)** |
...
#### 5.5.2 사용자가 보는 화면 흐름 (예: 3명 추가)
```
에이전트:  받는 사람을 검색하세요
          ┌─────────────────────────────┐
          │ [이름 또는 이메일 입력_____]  │   ← 검색창 카드
          │              [ 검색 ]         │
          └─────────────────────────────┘
사용자:    "김지훈" 입력 → 검색
(뒤에서:   Search for users("김지훈") 호출)
에이전트:  검색 결과 (체크해서 고르세요)
          ┌─────────────────────────────┐
          │ ☐ 김지훈 (마케팅팀)           │   ← 결과 체크리스트 카드
          │ ☐ 김지훈 (영업팀)             │
          │           [ 선택 완료 ]       │
          └─────────────────────────────┘
사용자:    ☑ 김지훈(마케팅팀) → 선택 완료
에이전트:  현재 1명 선택됨: 김지훈   [ 더 검색 ] [ 완료 ]
사용자:    더 검색          ← 다시 맨 위 검색창으로 돌아감(= 루프)
...        "박서연" → ☑ → "현재 2명" → 더 검색
...        "이수민" → ☑ → "현재 3명" → 완료
에이전트:  3명에게 공유를 진행합니다.  → 메인 흐름(문서 생성)으로
```
**루프의 정체**: `[더 검색]`을 누르면 흐름이 **맨 위 검색창으로 되돌아가고**, `[완료]`를 누르면 빠져나간다.

#### 5.5.3 토픽 노드 흐름
```mermaid
flowchart TD
  N1[Set variable: 통 비어있으면 초기화<br/>Global.RecipientsCsv] --> N2
  N2[Adaptive Card: 검색창<br/>Input.Text → SearchTerm] --> N3
  N3[Action: Office365Users<br/>Search for users V2 → SearchResults] --> N4{결과 있음?}
  N4 -- 없음 --> M[Message: 결과 없음] --> N2
  N4 -- 있음 --> N5[Adaptive Card: 결과 체크리스트<br/>ChoiceSet ← SearchResults → selectedRecipients]
  N5 --> N6[Set variable: 통에 이어붙이기<br/>RecipientsCsv += selectedRecipients]
  N6 --> N7{더 검색? / 완료?}
  N7 -- 더 검색 --> N2
  N7 -- 완료 --> DONE[루프 종료 → 메인 흐름<br/>RecipientsCsv 확정]
```
| 노드 | 하는 일 |
|------|---------|
| N1 | 최초 진입 시 통(`Global.RecipientsCsv`)을 빈 값으로 초기화 |
| N2 | **검색창 카드** 표시 → 입력값을 `SearchTerm`에 저장 |
| N3 | **Search for users (V2)** 호출 → 결과 배열 `SearchResults` |
| N4 | 결과 없으면 안내 후 N2로 |
| N5 | **결과 체크리스트 카드** 표시 → 체크한 UPN들을 `selectedRecipients`(콤마 구분)에 저장 |
| N6 | 통에 이어붙임 |
| N7 | "더 검색 / 완료" 질문 |
| N8(분기) | **더 검색 → N2 복귀(루프)** / **완료 → 종료** |

> 루프는 N7의 "더 검색" 분기를 **N2로 되돌려** 만든다. 통은 **전역 변수**라 재진입해도 값이 유지된다.

#### 5.5.4 핵심 Power Fx
**(a) 검색 결과 → 체크리스트 선택지** (N5 카드 `choices` 생성)
```
ForAll(
  Topic.SearchResults,
  { title: DisplayName & " (" & Department & ")",   // 화면 표시
    value: UserPrincipalName }                       // 저장값(권한 부여에 사용)
)
```
**(b) 통에 이어붙이기** (N6 Set variable)
```
If( IsBlank(Global.RecipientsCsv),
    Topic.selectedRecipients,
    Global.RecipientsCsv & "," & Topic.selectedRecipients )
```
- 다중 선택 `Input.ChoiceSet`은 체크된 값들을 `"a@x.com,b@x.com"`처럼 콤마로 묶어 반환 → 그대로 이어붙임.
...
```
**② 결과 체크리스트 카드** (choices는 §5.5.4(a)로 동적 생성)
```json
{ "type":"AdaptiveCard","version":"1.5",
...
```
- `style:"filtered"`(1.5)는 **방금 검색된 결과 목록**에 대한 클라이언트 타입어헤드 필터(전 직원 라이브 검색이 아님 — 5.5.1 제약).

#### 5.5.6 예외 처리
- **검색 결과 없음** → "결과가 없어요. 다시 입력" 후 검색창으로(N4).
- **아무것도 체크 안 함** → 이어붙이기 건너뜀.
- **중복 선택** → 최종 메인 플로우에서 `split → 중복 제거(union/distinct)`.
- **진행 안내** → N7에서 "현재 N명 선택됨"을 함께 표시.
- 게스트·비활성 계정은 후보에서 제외. 대본 참석자(`MinutesJson.attendees`)는 첫 화면 기본 후보로 제시.
...
| 7b | 권한 부여 | Grant access / Graph invite | SharePoint/Graph | RecipientsCsv 분할·중복제거 루프, roles=read, requireSignIn=true | failedRecipients | 부분 실패 허용 |
...
| 수신자 카드 검색 | 이름 일부 | 검색창 카드 → 결과 체크리스트 → 선택 → 더 검색 루프 |
| 권한 검증 | 비수신자 접근 | 열람 거부 |
...
| 2026-05-28 | 수신자 선택 카드 기반 구체화 — 네이티브 People Picker 제약, 검색창+결과 체크리스트 카드 2단 루프 | ms-design-agents |
| 2026-05-28 | 수신자 2단 카드 루프 상세화 — 화면 시나리오·노드 흐름·누적 전역변수·Power Fx(choices/누적)·예외 처리 | ms-design-agents |
