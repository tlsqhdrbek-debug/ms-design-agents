...
### 1.4 핵심 설계 관점
"대본 → 회사 양식 회의록"으로의 변환은 **생성형 AI(GPT)** 가 수행한다. 이 작업은 Copilot Studio의 **Prompt action(AI Builder 프롬프트)** 으로 구현하며, 외부 LLM 기반이라 외부망 배치가 자연스럽다. 회의 대본이 외부 모델로 입력된다는 점은 §7의 보안 조건으로 통제한다.
...
| Prompt action | 대본 → 구조화 JSON 회의록 | 외부망 | Copilot Studio Prompt(AI Builder, GPT) |
...
sequenceDiagram
  ...
  T->>P: Transcript 전달
  P-->>T: MinutesJson (구조화 회의록)
  T->>U: 초안 표시 + "확정/수정?" (Message+Question)
  U->>T: 수정/확정
  T->>U: 수신자 선택 (Question / Adaptive Card)
  U->>T: Recipients
  T->>F: Call an action — 입력(MeetingTitle, MinutesJson, OwnerUpn, RecipientsCsv ...)
  Note over F: Word 채우기→저장→상속차단→권한부여
...
| 매개변수 | 타입 | 설명 |
|---------|------|------|
| MeetingTitle | Text | 회의명 |
| MeetingDate | Text | 회의 일자(YYYY-MM-DD) |
| TeamName | Text | 팀/부서명(저장 경로용) |
| MinutesJson | Text | 구조화 회의록(JSON 문자열, §10 스키마) |
...
| 2 | **Question** (대본) | "회의 대본을 붙여넣어 주세요." 멀티라인 텍스트 수신 | → `Transcript` (Text) |
| 3 | **Question/Prompt** (메타) | 제목·일자·팀 입력받기 또는 Prompt로 대본에서 추출 | → `MeetingTitle`, `MeetingDate`, `TeamName` |
| 4 | **Action → Create a prompt** | 대본을 회사 양식 구조화 회의록(JSON)으로 변환 (§6) | `Transcript` → `MinutesJson` (Text) |
| 5 | **Message** (초안 표시) | `MinutesJson`을 보기 좋게 렌더(Adaptive Card 권장) | `MinutesJson` |
| 6 | **Question** (검토) | "이대로 확정할까요? 수정할 내용이 있나요?" 선택지: 확정 / 수정 | → `ReviewChoice` |
| 7 | **Condition** | 수정 선택 시 수정 내용 받아 Prompt 재실행(4로 루프), 확정 시 다음 | - |
| 8 | **Question** (수신자) | 대본에서 추출한 참석자를 **기본 후보로 제시**, 다중 선택. 외부 게스트 제외 | → `RecipientsCsv` (Text) |
| 9 | **Action** (플로우 호출) | 회의록 문서생성·공유 플로우 호출. §4.4 입력 매핑 | 입력 6종 → `Status`, `FileWebUrl`, `FailedRecipients` |
| 10 | **Condition** | `Status`에 따라 분기 | - |
| 11 | **Message** (종료) | 성공: "회의록이 공유되었습니다. [열기](FileWebUrl)" / 부분실패: 실패자 안내 | - |

### 5.3 변수 목록
| 변수 | 타입 | 범위 | 용도 |
|------|------|------|------|
| Transcript | Text | Topic | 붙여넣은 대본 |
| MeetingTitle/MeetingDate/TeamName | Text | Topic | 메타·저장 경로 |
| MinutesJson | Text | Topic | Prompt 출력(구조화 회의록) |
| ReviewChoice | Text | Topic | 검토 분기 |
...
Microsoft 문서상 Prompt(prompt builder)의 대표 사용처가 "**transcripts 요약**, action item 추출, 분류, 번역, 데이터 변환"이며, 본 요구와 정확히 일치한다. 또한 MS는 GPT 출력에 대한 **사람 검토(human oversight)** 를 권고 → §5.2의 검토 노드(6)와 부합.

### 6.2 프롬프트 설계 (요지)
- **지시(instruction)**: "다음 회의 대본을 회사 표준 회의록 양식으로 정리하라. 반드시 아래 JSON 스키마로만 출력하라. 추측하지 말고 대본에 없는 내용은 비워라."
- **컨텍스트(context)**: `Transcript` 입력 변수.
- **출력**: §10의 JSON 스키마(Text). 토픽 변수 `MinutesJson`에 저장.
- 입력/출력 매개변수에 **명확한 이름·설명**을 부여(스키마 매칭 안정화).
...
|-----|------|------|--------|------|------|------|
| 1 | 트리거 | When an agent calls the flow | Copilot | §4.4 입력 6종 | - | 입력 매개변수 정의 |
| 2 | 회의록 파싱 | Parse JSON | 내장 | MinutesJson | minutes | §10 스키마 |
| 3 | 파일명 | Compose | 내장 | `MeetingDate_MeetingTitle.docx` | fileName | 금칙문자 치환 |
| 4 | 문서 작성 | **Populate a Microsoft Word template** | Word Online(Business) | 템플릿(사이트/라이브러리/파일) + 필드 매핑 | docxContent | 콘텐츠 컨트롤 채움 |
| 5 | 저장 | Create file | SharePoint | `/회의록/YYYY/팀명/fileName`, docxContent | itemId, webUrl | 전용 라이브러리 |
...
2. 커넥터(Word Online Business, SharePoint, Teams, HTTP/Graph) 활성화 및 DLP 확인.
...
2. **에이전트 플로우 작성** — `When an agent calls the flow` 트리거에 입력 6종 정의 → §8.2 액션 구성 → `Respond to the agent`에 출력 3종 정의(async off) → **게시**.
3. **Prompt action 작성** — Create → Prompt, 지시·입력(Transcript)·출력(JSON), Test prompt로 검증.
4. **에이전트·토픽 작성** — §5.2 노드 구성. Action 노드(9)에서 게시된 플로우 선택 후 **입력/출력 변수 매핑**.
...
| 2026-05-28 | Copilot Studio↔Power Automate 연계 상세 보강 (트리거/응답 액션·노드별 구성·Prompt action·변수 매핑·100초/Refresh 운영주의) — MS Learn 근거 반영 | ms-design-agents |
